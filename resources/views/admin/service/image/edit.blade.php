...
@section('admin_title')
    แก้ไขรูปภาพ {{$service->service_name}}
@endsection
        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title">แก้ไขรูปภาพ {{$service->service_name}}</h3>
                    <div class="box-tools pull-right" data-toggle="tooltip" title="Status"></div>
                </div>
                <div class="box-body">

                    <form id="form-update-service-image" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group mb-3">
                            <label class="col-form-label">ชื่อรูปภาพ</label>
                            <input class="form-control" name="service_image_name" value="{{$service_image->service_image_name}}" type="text" required
                                   placeholder="">
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="file" id="imageUpload" name="service_image_file_name" class="form-control"
                                   accept="image/*">
                            <input type="hidden" name="old_service_image_file_name" value="{{$service_image->service_image_file_name}}">
                            <small class="text-danger" id="msg_file"></small>
                        </div>

                        <div class="row mb-3">
                            <div class="col-md-12 mb-3">
                                <img  src="{{url('/images/service', $service_image->service_image_file_name)}}" class="profile-img" id="imagePreview">
                            </div>
                        </div>

                        <div class="form-group mb-3">
                            <label class="col-form-label">คำอธิบายรูปภาพ</label>
                            <textarea class="form-control" rows="5" name="service_image_description">{{$service_image->service_image_description}}</textarea>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control" name="service_image_sequence"
                                   type="number"
                                   value="{{$service_image->service_image_sequence}}"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control" name="service_image_status">
                                <option value="active" @if ($service_image->service_image_status == 'active') selected @endif>เผยแพร่</option>
                                <option value="inactive" @if ($service_image->service_image_status == 'inactive') selected @endif>ไม่เผยแพร่</option>
                            </select>
                        </div>
...
@section('admin_script')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#form-update-service-image').formValidation({
                framework: 'bootstrap',
                fields: {
                    service_image_name: {
                        validators: {
                ...
                .on('success.form.fv', function (event) {
                    ...
                    let service_image_uid = `{{$service_image->uid}}`;
                    $.ajax({
                        url: `{{url("administrator/service/")}}/${service_uid}/show/images/${service_image_uid}/update`,
                        method: 'POST',
                        data: formData,
                        processData: false,
                        contentType: false,
