        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title">การให้บริการ จำนวน <label id="count-total">{{$services->total()}}</label> สไลด์</h3>

                    <div class="box-tools pull-right" data-toggle="tooltip" title="Status">
                        <div class="btn-group" data-toggle="btn-toggle">
                            <a href="{{url('administrator/service/create')}}" class="btn btn-primary btn-sm">เพิ่มการให้บริการ</a>
                        </div>
                    </div>
                ...
                <div class="box-body">
                    <form method="get">
                        <div class="row row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-sm-2 row-cols-2 mb-3">
                            <div class="col-md-4">
                                <select name="service_status" id="search_status" class="form-control form-select"
                                        aria-label="Default select example">
                                    <option value="">สถานะการแผยแพร่</option>
                                    <option value="inactive"
                                            @if ($request->service_status == 'inactive') selected @endif>ปิด
                                    </option>
                                    <option value="active" @if ($request->service_status == 'active') selected @endif>
                                        เปิด
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="table table-responsive">
                        <table class="table" style="width: 150%;">
                            ...
                            <tr style="text-align: left; font-weight: bold;">
                                ...
                                <th style="width: 19%;">ชื่อ</th>
                                <th style="width: 15%;">เบอร์โทรศัพท์ติดต่อ</th>
                                <th style="width: 15%;">ชื่อผู้ติดต่อ</th>
                                <th style="width: 5%; text-align: center;">ลำดับแสดงผล</th>
                                <th style="width: 10%; text-align: center;">สถานะ</th>
                                <th style="width: 12%; text-align: left;">วันที่เพิ่ม</th>
                                <th style="width: 12%; text-align: left;">อัพเดตล่าสุด</th>
                            </tr>
                            </thead>
                            ...
                            @foreach($services as $service)
                                <tr style="text-align: center;">
                                    <td valign="top">
                                        <a class="btn btn-primary btn-air-primary px-2" title="ดูข้อมูล"
                                           href='{{url("administrator/service/$service->uid/show")}}'><i
                                                    class="fa fa-eye"></i></a>
                                        <a class="btn btn-primary btn-air-primary px-2" title="แก้ไขข้อมูล"
                                           href='{{url("administrator/service/$service->uid/edit")}}'><i
                                                    class="fa fa-edit"></i></a>
                                        <button class="btn btn-danger btn-air-danger px-2 btn-delete-service"
                                                data-id="{{$service->uid}}"><i class="fa fa-trash"></i>
                                        </button>
                                    </td>

                                    <td align="left" valign="top">{{$service->service_name}}</td>
                                    <td align="left" valign="top">
                                        {{$service->service_contact_phone_number ? $service->service_contact_phone_number : ''}}
                                    </td>
                                    <td align="left" valign="top">
                                        {{$service->service_contact_name ? $service->service_contact_name : ''}}

                                    </td>
                                    <td align="center"
                                        valign="top">{{$service->service_sequence ?  $service->service_sequence : '-'}}</td>
                                    ...
                                    <td align="center" valign="top">{{formatDateThat($service->created_at)}}</td>
                                    <td align="center" valign="top">{{formatDateThat($service->updated_at)}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-delete-service', function () {
                ...
                swal({
                    title: "ยืนยันการลบ",
                    text: "คุณต้องการลบการให้บริการนี้หรือไม่",
                    icon: "warning",
                    buttons: {
                        cancel: "ไม่ใช่",
                        catch: {
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $(".loading").show();
                            $.ajax({
                                url: `{{url("/administrator/service")}}/${id}/delete`,
                                method: 'POST',
                                data: {
