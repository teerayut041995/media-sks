...
//            ->orderBy('sub_category_ranking', 'asc')
    ...
    public function recentPosts($limit = 8)
    {
        return Post::where('post_status', '1')
            ->select([
                'posts.id', 'posts.uid', 'posts.user_id', 'posts.category_id', 'posts.sub_category_id','posts.post_title','posts.post_slug','posts.post_image','posts.post_detail','posts.post_status', 'posts.created_at'
            ])
